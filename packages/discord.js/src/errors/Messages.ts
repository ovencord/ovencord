...
export const Messages = {
  [ErrorCodes.ClientInvalidOption]: (prop: any, must: any) => `The ${prop} option must be ${must}`,
  ...
  [ErrorCodes.ClientNotReady]: (action: any) => `The client needs to be logged in to ${action}.`,
  ...
  [ErrorCodes.BitFieldInvalid]: (bit: any) => `Invalid bitfield flag or number: ${bit}.`,
  ...
  [ErrorCodes.ShardingShardNotFound]: (id: any) => `Shard ${id} could not be found.`,
  [ErrorCodes.ShardingAlreadySpawned]: (count: any) => `Already spawned ${count} shards.`,
  [ErrorCodes.ShardingProcessExists]: (id: any) => `Shard ${id} already has an active process.`,
  [ErrorCodes.ShardingWorkerExists]: (id: any) => `Shard ${id} already has an active worker.`,
  [ErrorCodes.ShardingReadyTimeout]: (id: any) => `Shard ${id}'s Client took too long to become ready.`,
  [ErrorCodes.ShardingReadyDisconnected]: (id: any) => `Shard ${id}'s Client disconnected before becoming ready.`,
  [ErrorCodes.ShardingReadyDied]: (id: any) => `Shard ${id}'s process exited before its Client became ready.`,
  [ErrorCodes.ShardingNoChildExists]: (id: any) => `Shard ${id} has no active process or worker.`,
  ...
  [ErrorCodes.ColorConvert]: (color: any) => `Unable to convert "${color}" to a number.`,

  [ErrorCodes.InviteOptionsMissingChannel]:
    'A valid guild channel must be provided when GuildScheduledEvent is EXTERNAL.',

  [ErrorCodes.InteractionCollectorError]: (reason: any) =>
  ...
  [ErrorCodes.VoiceStateInvalidType]: (name: any) => `${name} must be a boolean.`,
  ...
  [ErrorCodes.FetchOwnerId]: (type: any) => `Couldn't resolve the ${type} ownerId to fetch the ${type} member.`,

  [ErrorCodes.InvalidType]: (name: any, expected: any, an = false) => `Supplied ${name} is not a${an ? 'n' : ''} ${expected}.`,
  [ErrorCodes.InvalidElement]: (type: any, name: any, elem: any) => `Supplied ${type} ${name} includes an invalid element: ${elem}`,
  ...
  [ErrorCodes.ThreadInvitableType]: (type: any) => `Invitable cannot be edited on ${type}`,
  ...
  [ErrorCodes.GuildUncachedEntityResolve]: (type: any) =>
  ...
  [ErrorCodes.CommandInteractionOptionNotFound]: (name: any) => `Required option "${name}" not found.`,
  ...
  [ErrorCodes.ModalSubmitInteractionComponentNotFound]: (customId: any) =>
  ...
  [ErrorCodes.NotImplemented]: (what: any, name: any) => `Method ${what} not implemented on ${name}.`,
  ...
  [ErrorCodes.PermissionOverwritesTypeMismatch]: (expected: any) =>
